...
module.exports = () => {
      ...
      if (err) {
        ...
        db.query(categoryStr, (err1, data1) => {
          if (err) {
            ...
            let cateName =
              data1 && data1.length > 0 && data1[0].category_name
                ? data1[0].category_name
                : "-";
            if (data.length == 0) {
                ...
                Flow.findAll().then((flows) => {
                  flows.forEach((ielem) => {
                    ...
                    if (
                      ...
                      flowObj.product_id == proData.product_id
                    ) {
                  ...
                  res
                    .status(200)
                    .send({ code: 200, msg: "查询成功！", data: proData })
                    .end();
                });
                //浏览器表添加浏览纪录
    ...
    History.findAll().then((Historys) => {
      ...
      Historys.forEach((ielem) => {
        ...
        if (
          ...
          historyObj.product_id == productId
        ) {
          addTF = false; //有关注
        ...
        db.query(cartStr, (err, data) => {
          if (err) {
            ...
            if (data.length == 0) {
              res
                .status(200)
                .send({ code: 200, msg: "查询成功！", cartNum: 0, data: [] })
                .end();
            } else {
              res
                .status(200)
  ...
  route.post("/reg", (req, res) => {
    ...
    User.findAll().then((res1) => {
      ...
      if (!addTF) {
        ...
        User.create({
          user_name: mObj.regName,
          user_namesub: mObj.regnamesub,
          login_password: regPasswd,
          user_number: "123",
        }).then(function (user) {
        ...
        if (data.length == 0) {
          ...
          if (dataw.login_password === password) {
            ...
            Token.findAll().then((Tokens) => {
              ...
              let tokenT = jwt.sign(
                { user_name: dataw.user_name, user_id: dataw.user_id },
                PRIVATE_KEY,
                { expiresIn: JWT_EXPIRED }
              );
              if (addTF) {
                ...
                Token.create({
                  token: tokenT,
                  user_name: dataw.user_name,
                  user_id: dataw.user_id,
                  user_namesub: dataw.user_namesub,
                ...
                Token.update(
                  ...
                  res
                    .status(200)
                    .send({ msg: "登录成功！", code: 200, data: dataw })
                    .end();
                });
              }
            });
  ...
  route.post("/editUser", (req, res) => {
    ...
    let editObj = {
      ...
      user_namesub: userInfoObj.user_namesub,
    };
  ...
  //用户信息修改
  route.post("/editUserApp", (req, res) => {
    let userInfoObj = {};
    userInfoObj = req.body;
    // for (let obj in req.body) {
    //   userInfoObj = Object.assign(userInfoObj, JSON.parse(obj));
    // }
  ...
  route.post("/addCart", (req, res) => {
    ...
    Cart.findAll().then((res1) => {
      res1.forEach((ielem, indOne) => {
        if (
          ielem.dataValues.product_id == cartObj.product_id &&
          cartObj.user_id == ielem.dataValues.user_id
        ) {
      ...
      if (addTF) {
        Cart.create({
          ...
          res
            .status(200)
            .send({ msg: "创建购物车成功！", data: cart, code: 200 })
            .end();
        });
      }
    });
  ...
  route.post("/delCart", (req, res) => {
    ...
    Cart.destroy({
      ...
      res
        .status(200)
        .send({ msg: "购物车删除成功！", data: cart, code: 200 })
        .end();
    });
  });
  //购物车修改产品数量
  route.post("/changeCartNum", (req, res) => {
    ...
    Cart.update(
      ...
      res
        .status(200)
        .send({ msg: "修改数量成功！", data: cart, code: 200 })
        .end();
    });
  });
  //购物车修改产品数量APP
  route.post("/changeCartNumApp", (req, res) => {
    let cartObj = {};
    // for (let obj in req.body) {
    //   cartObj = JSON.parse(obj);
    // }
  ...
  route.get("/cartNum", (req, res) => {
    ...
    getToken(req.headers.accesstoken)
        ...
        db.query(cartStr, (err, data) => {
          if (err) {
            ...
            if (data.length == 0) {
              res
                .status(200)
                .send({ code: 200, msg: "查询成功！", cartNum: 0 })
                .end();
            } else {
              res
                .status(200)
                .send({ code: 200, msg: "查询成功！", cartNum: data.length })
                .end();
            }
          }
        });
      ...
      .catch((err) => {
        res
          .status(200)
          .send({ code: 200, msg: "用户未登录！", cartNum: 0 })
          .end();
      });
  });
  //用户关注产品数量
    ...
    getToken(req.headers.accesstoken)
      .then((userInfo) => {
        Flow.findAll({
          ...
          res
            .status(200)
            .send({ code: 200, msg: "查询成功！", flowNum: flows.length })
            .end();
        });
      })
      .catch((err) => {
        res
          .status(200)
          .send({ code: 200, msg: "用户未登录！", flowNum: 0 })
          .end();
      });
  });
  //用户关注产品
        ...
        db.query(flowStr, (err, data) => {
          if (err) {
            ...
            if (data.length == 0) {
              res
                .status(200)
                .send({ code: 200, msg: "查询成功！", flowNum: 0, data: [] })
                .end();
            } else {
              res
                .status(200)
    ...
    getToken(req.headers.accesstoken)
      .then((userInfo) => {
        History.findAll({
          ...
          res
            .status(200)
            .send({ code: 200, msg: "查询成功！", historyNum: historys.length })
            .end();
        });
      })
      .catch((err) => {
        res
          .status(200)
          .send({ code: 200, msg: "用户未登录！", historyNum: 0 })
          .end();
      });
  });
  //用户浏览产品
        ...
        db.query(flowStr, (err, data) => {
          if (err) {
            ...
            if (data.length == 0) {
              res
                .status(200)
                .send({ code: 200, msg: "查询成功！", historyNum: 0, data: [] })
                .end();
            } else {
              res
                .status(200)
  ...
  route.post("/proManage", (req, res) => {
    ...
    let categorySQL =
      flowProObj.cateId != ""
        ? `products.category_id = ${flowProObj.cateId}`
        : `1=1`;
    // 查询所有产品
    const proManageSQL = `
            SELECT
            ...
            limit ${(flowProObj.pageNum - 1) * flowProObj.pageSize},${
      flowProObj.pageSize
    }
    ...
    getToken(req.headers.accesstoken)
      .then((userInfo) => {
        Category.findAll().then((categories) => {
          res
            .status(200)
            .send({ code: 200, msg: "查询成功！", data: categories })
            .end();
        });
      })
      .catch((err) => {
  ...
  route.post("/shoplist", (req, res) => {
    ...
    const shopListSQL = `
        ...
        limit ${(shopProObj.pageNum - 1) * shopProObj.pageSize},${
      shopProObj.pageSize
    }
        `;
  ...
  route.post("/userList", (req, res) => {
    ...
    const userSQL = `
        ...
        FROM
            users
        limit ${(userProObj.pageNum - 1) * userProObj.pageSize},${
      userProObj.pageSize
    }
        `;
  ...
  route.post("/addUser", (req, res) => {
    ...
    let editObj = {
      ...
      user_namesub: userObj.user_namesub,
    };
    if (userObj.password != "") {
      editObj.login_password = editPassword;
    }
